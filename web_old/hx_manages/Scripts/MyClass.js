...
$(document).ready(function ()
{
   ...
   // if (UserLevel==2){
          ...
          { field: 'creator', title: '创建人', width: 70, sortable: true, align: 'center',
                formatter: function (value, row, index){
                    for (var i in creator_name){
                         if (creator_name[i].id == value){
                            return creator_name[i].realname; 
							break;
                        } 
                    }
                }
    ...
    pager = $('#ClassManager').datagrid("getPager");
    pager.pagination({

        onSelectPage: function (pageNumber, pageSize) {
            $(this).pagination('loading');
            var ltemp = (pageNumber - 1) * pageSize;
            LoadClassList(ltemp, pageSize);
	 ...
	 // 查找学生信息
    function LoadClassList(s, l) {
        var parameValue = {};
        var tempValue =null;

        tempValue = $('#txtClassName').val();
		if (tempValue=="输入班级名称") {tempValue="";}
        if ($('#txtClassName').attr('innt') != 1) {
            if ($.trim(parameValue).length > 0) {
                parameValue.classname = tempValue;
            }
        }
        ...
        var ajaxParames = {
            url: Webversion + '/class/list/' + s + ',' + l + '/withEvaluation' ,
            type: "GET",
            dataType: "json",
            data: parameValue,
            success: function (result) {
                ...
                var reacodeList = [], total = 20;
				student = result.student;
				teacher = result.teacher;
                if (result != null && result["class"] != null) {
                    $.each(result["class"], function (i, n) {
                        var tempItem = {};
						tempItem.id = n.id;
						
                        tempItem.Name = n.Name;
                        tempItem.class_type = n.class_type;
                        tempItem.creator = n.creator;
                        tempItem.creat_date = n.creat_date;
						
                        tempItem.is_open = n.is_open;
                        tempItem.remark = n.remark;	
						 
						
						tempItem.studentnum =0;//result.student[i].total;
                        tempItem.teachernum =0;//result.teacher[i].total;	
						for (var j in  student){
                        if (student[j].class_Id == n.id){tempItem.studentnum =student[j].total;	}
                        } 
						
						for (var j in teacher){
                        if (teacher[j].class_Id == n.id){tempItem.teachernum =teacher[j].total;	}
                        } 
                ...
                $('#ClassManager').datagrid("loadData", dataGrid);
        ...
        columns: [[
           ...
           /*  { field: 'grade_id', title: '年级', width: 65, sortable: true, align: 'center',
            ...
            { field: 'founder', title: '平均成绩', width: 90, sortable: true, align: 'center',
				...
				//score = result["score"];
				var avg = 0;
				//alert(row.student_id);
				//alert(score[0].student_id);
			 if (score != null) {
					
                     $.each(score, function (i, f) {
                         if (row.student_id == score[i].student_id) {
							if(score[i].avg_score == null){
								return avg='-';
							  }
                              avg = parseInt(score[i].avg_score);
                            }
                      });
					  return avg; 
...
function initClass()
{
    ...
    $.ajax({
        ...
        success: function (result)
        {
            ...
            teacher = result.teacher;
            creator_name = result.name;
           $('# ').datagrid("loadData", result["class"]);
