...
$(document).ready(function () {
    ...
    $('document').KeyInput($("#SchoolText"), '请输入姓名');
     var tempcolumns = [[
		    { field: 'teacherUName', title: '派送日期', width: 80, align: 'center', sortable: true },
			{field: 'TeacherRName', title: '测评/作业名称', width: 220, align: 'center'},
			{ field: 'TeacherSubject', title: '学科', width: 60, align: 'center', sortable: true },
			{ field: 'TeacherRole', title: '派送教师', width: 80, align: 'center', sortable: true },
			{ field: 'TeacherEmail', title: '派送目标对象', width: 120, align: 'center', sortable: true },
            { field: 'creat_date', title: '是否通知', width: 80, align: 'center', sortable: true },
            {
                field: 'c_id', title: '操作', align: 'center',
                formatter: function (value, row, index) {
                    var s = "<a href=\"#\" style='color:blue;' onclick=\"SelectSchoolName('" + value + "'," + index + ")\">发送短信</a>&nbsp;<a href=\"#\" style='color:blue;' onclick=\"EditSchoolName('" + value + "'," + index + ")\">查看通信录</a>";
                    return s;
                }
            }
        ]];
	...
	var fitflag = true;
	var toolbar = '#SerToolBar_teacher';
	var obj = LoadSchool;
                ...
                handler: function () {
                    ...
                    $.ajax({
                        ...
                        error: function (result) {
                            $.messager.progress("close");
                            $.messager.alert('错误', '新增人员创建失败!<br/><span style="color:red;">可能原因：被指定的校区负责人不是教师身份或系统中已存在此校区。</span>', 'warning');
                        }
                    });
        ...
        buttons: [{
            ...
            handler: function () {
                ...
                $.ajax({
                    ...
                    error: function (result) {
                        $.messager.progress("close");
                        $.messager.alert('错误', '添加校区负责人失败!<br/><span style="color:red;">可能原因：此校区下可能已存在此负责人。</span>', 'waining');
                    }
                });
