...
function Longding(result){
	...
	if(result.list!=null){
		...
		$.each(result.list,function(i,s){
				...
				if(s.creator_date!=null){
					itemtemp.creat_Time = s.creator_date.substring(0,10);
				}
...
function noneclass_stu(past_id){
	...
	var datacc = {'action':'other_stu','condition':"center_id^"+centerAll.center_id+"$zone_id^"+zone_id+"$new_student_status^"+past_id,'noclass_stu':1,'center_id':centerAll.center_id,'zone_id':zone_id};
	...
	$('#past_BtnSearch_stu').bind('click',function(){
		...
		if(past_name!=null&&past_name!=""&&past_name!='请输入姓名'){
			var dataccs = {'action':'other_stu','condition':"center_id^"+centerAll.center_id+"$zone_id^"+zone_id+"$new_student_status^"+past_id+'$realname@'+past_name,'noclass_stu':1,'center_id':centerAll.center_id,'zone_id':zone_id};	
		...
		if(zone_stu_nums==0){
			if(max_counts!=null&&max_counts!=""&&max_counts!=0){
				if(parseInt(max_counts)<=parseInt(reality_counts)){
					$.messager.alert('温馨提示','学生上限：'+max_counts+'<br />入班学生：'+reality_counts+'<br />如果学生添加班级，请于供应商联系!','info');
			...
			if(values_stus.length>(parseInt(zone_stu_nums)-parseInt(currite_zone_stu_nums))){
				$.messager.alert('温馨提示','学生上限：'+zone_stu_nums+'<br />入班学生：'+currite_zone_stu_nums+'<br />如果学生添加班级，请于供应商联系!','info');
			}else{
				$.each(values_stus,function(i,n){
					values_stus_json.push({'class_stu_id':n.class_stu_id,'student_id':n.student_id,'center_id':n.center_id,'zone_id':n.zone_id,'class_id':n.class_id,'stu_user_id':n.stu_user_id,'student_username':n.stu_Real,'student_user_id':n.stu_user_id});
				});
	 ...
	 /*---循环验证begin*/
	  ...
	  if(bigclassname!='请选择'||teacher_list!=""){
		  ...
		  if(zone_stu_nums==0){
			  if(max_counts!=null&&max_counts!=""&&max_counts!=0){
				  if(parseInt(max_counts)<=parseInt(reality_counts)){
					  $.messager.alert('温馨提示','学生上限：'+max_counts+'<br />入班学生：'+reality_counts+'<br />现只能添加学生,不能入班操作;<br />如果学生添加班级，请于供应商联系!','info');
					  return false;  
				  }
			  }
		  }else{ //不等于0
...
function updateclasshand(rowData,listcss_id){
	...
	if(type_class_radio==1){ 
		...
		if(addstuclass!='请选择'){
			var adddata = {};
			if(listcss_id!='#noneclass_stu'){
				adddata = {'action':'join','center_id':rowData.center_id,'zone_id':rowData.zone_id,'student_id':rowData.student_id,'class_id':addstuclass,'nonetype':nonetype,'class_stu_id':rowData.class_stu_id};
			}else{
				...
				adddata = {'action':'join','center_id':centerAll.center_id,'zone_id':$('#A_zones',window.parent.document).find("option:selected").val(),'stus_info':rowData};	
			}
		...
		if(addteacherName_smail!='请选择'){
			...
			if(listcss_id!='#noneclass_stu'){
				...
				adddata = {'action':'batch_add_small_class','center_id':centerAll.center_id,'zone_id':$('#A_zones',window.parent.document).find("option:selected").val(),'stus_info':rowData};	
			}
