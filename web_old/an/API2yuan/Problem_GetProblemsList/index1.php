		...
		public function onsucess(){
			...
			if($rs[0]){
				...
				switch($_REQUEST['cateryid']){
						...
						case 2:
							if($_REQUEST['years'] == 0){
								$this -> selectandwrite('*','exam_examination',null,null,'exam_type=1  and name like BINARY "%'.$_REQUEST['area'].'%"  and '.$subject.' and '.$grade.' limit '.$startrecord.','.$limit);
							}
							else{
								$this -> selectandwrite('*','exam_examination',null,null,'exam_type=1  and name like BINARY "%'.$_REQUEST['area'].'%" and name like "%'.$_REQUEST['years'].'%" and '.$subject.' and '.$grade.' limit '.$startrecord.','.$limit);
							}
