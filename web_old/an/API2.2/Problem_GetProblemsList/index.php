		...
		public function onsucess(){
			...
			if($rs[0]){
				...
				if($_REQUEST['subjectid']==0){
					if($_REQUEST['cateryid']==4 || $_REQUEST['cateryid']==5){
						$subject = " exam_question.subject_id!=0";
					}
					else{
					...
					if($_REQUEST['cateryid']==4 || $_REQUEST['cateryid']==5){
						$subject = " exam_question.subject_id=".$_REQUEST['subjectid'];
					}
					else{
				...
				/* switch($_REQUEST['cateryid']){
						...
						//错题本
						case 4:
							$this -> selectandwrite('exam_question.*,study_collection.add_time','(exam_question JOIN study_collection ON study_collection.question_id=exam_question.id)',null,null,'user_id='.$id.' and flag=1 and '.$subject.' order by study_collection.add_time desc limit '.$startrecord.','.$limit);
						...
						//好题本
						case 5:
							$this -> selectandwrite('exam_question.*,study_collection.add_time','(exam_question JOIN study_collection ON study_collection.question_id=exam_question.id)',null,null,'user_id='.$id.' and flag=2 and '.$subject.' order by study_collection.add_time desc limit '.$startrecord.','.$limit);
							for($i=0;$i<count($this -> arr["content"]);$i++){
								$this -> arr["content"][$i] = base64_encode($this -> arr["content"][$i]);
								$this -> arr["answer"][$i] = base64_encode($this -> arr["answer"][$i]);
							}
				...
				if($_REQUEST['cateryid']==4 or $_REQUEST['cateryid']==5){
					$this -> arr["sc"] = 200;
					$this -> arr["id"][] = 0;
					...
					$this -> arr["add_time"][] = "请升级";
					$this -> arr["answer"][] = base64_encode("请升级到最新题库");
					$this -> arr["counts"] = 1;
					$this -> arr["content"][] = base64_encode("请升级到最新题库");
				}
				else{
					$this -> arr['name'][] = '请升级';
					$this -> arr['knowledge_list'][] = '请升级';
					$this -> arr['id'][] = 0;
					$this -> arr['grade_id'][] = $_REQUEST['grade_id'];
					$this -> arr['sc'] = 200;
					$this -> arr["counts"] = 1;
					$this -> arr["subject_id"][] = 1;
