...
$(document).ready(function(){
	...
	if(UserInfo!=null&&UserInfo!=undefined){
		...
		$('#back_html').click(function(){
			window.location = './exercise_zhenti.html';	
		});
		...
		exerciseHandler.examExercise.loadExamData(hrefParams['zhenti_id'] ,hrefParams['subject_id'] , function(flag , data){
			if(flag){
	...
	//					var answer_clone = $('#answer_objective_template').clone().attr('id' , null);
		...
		$('.tmp_save').click(function(){
			...
			var checkpass = true;
			var examResult = {};
			examResult.exercise_id = hrefParams['zhenti_id'];
			...
			examResult.exam_type = 3;  //  1.作业/测试 2. 专题  3. 真题 4.名校
		...
		$('.submit_btn').click(function(){
			...
			exerciseHandler.examExercise.uploadExamResult(JSON.stringify(examResult), function(flag , data){
				...
				if(flag){
					...
					$('.exam_over_Scontent').html('<li>'+parseInt(examResult.my_score)*100+'%</li><li>--</li><li>--</li><li>'+ssstime+' 分钟</li><li>-- 分钟</li>');
					...
					if(reTemp.list!=null&&reTemp.list!=""){
						$.each(reTemp.list,function(re_i,re_n){
							if(re_i!=0){
								...
								rehtmls += '<span>'+create_date+' 完成 得分率 '+parseInt(re_n.my_score)*100+'%</span>';
							}
						});
		...
		*/
		...
		$('.download_word').click(function(){
			window.open('/restAPI3.0/get_word?action=zhenti&zhenti_id='+hrefParams['zhenti_id']+'&subject_id='+hrefParams['subject_id']);
		});
