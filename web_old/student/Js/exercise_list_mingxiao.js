	...
	if(UserInfo!=null&&UserInfo!=undefined){
		...
		if(section_idT==3){
			...
			exerciseHandler.query('nav', {type:pageType,'section_id':section_idT,'limit':limit,'user_id':$.cookie("UsersId")} , function(flag , data){
				var label = {'zhenti':'真题','zhuanti':'专题','mingxiao':'名校'};
				if(flag){
		...
		//							console.log(data.subject[i]);
			...
			$(document).delegate('.nav_element','click',function(){
				...
				hrefParams['type'] = hrefParams['type'] ? hrefParams['type'] : 2;
				//省份赋值
			...
			$(document).delegate('.zhuanti_enter','click',function(){
				...
				if(pageType == 'mingxiao'){
					window.location.href = 'exercise_'+pageType+'_do.html?'+pageType+'_id='+$(this).attr('exercise_id')+'&subject_id='+selectSubjectID+'&study_exercise_id='+$(this).attr('study_exercise_id');
				}else{
			...
			//没有试卷的时候
			layer.alert('高中才有名校试卷!',9,'温馨提示',function(){
