	...
	class crest extends REST{
		...
		public function getList(){
			...
			if(strlen($_REQUEST['condition'])!=0){
				...
				$this -> db -> sql = "select ".$str." from area_province where ".$condition." limit ".$this -> page[0].",".$this -> page[1];
			}
			else{
				$this -> db -> sql = "select ".$str." from area_province limit ".$this -> page[0].",".$this -> page[1];
			}
			
			$this -> db -> Query();
			$this -> b['province']['info'] = $this -> db -> rs;
			$this -> b["sc"] = 200;
		}
